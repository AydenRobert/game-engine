...
void geometry_deduplicate_vertices(u32 vertex_count, vertex_3d *vertices,
    ...
    // Create a new array
    void *raw_unique_verts =
        kallocate(sizeof(vertex_3d) * vertex_count + 15, MEMORY_TAG_ARRAY);
    vertex_3d *unique_verts = (void *)get_aligned((u64)raw_unique_verts, 16);
    *out_vertex_count = 0;
    ...
    for (u32 v = 0; v < vertex_count; v++) {
        ...
        for (u32 u = 0; u < *out_vertex_count; u++) {
            if (vertex3d_equal(vertices[v], unique_verts[u])) {
                // Reassign
                reassign_index(index_count, incices, v - found_count, u);
                found = true;
                found_count++;
                break;
            }
    ...
    *out_vertices =
    ...
    KDEBUG(
        "geometry_deduplicate_vertices - removed %d vertices. Orig/Now: %d/%d",
