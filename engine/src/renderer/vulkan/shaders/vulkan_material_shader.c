...
#define ATTRIBUTE_COUNT 2
    ...
    if (!vulkan_graphics_pipeline_create(
            context, &context->main_renderpass, sizeof(vertex_3d),
            ATTRIBUTE_COUNT, attribute_descriptions,
            descriptor_set_layout_count, layouts, MATERIAL_SHADER_STAGE_COUNT,
    ...
    if (!vulkan_buffer_create(
            context, sizeof(vulkan_material_shader_global_ubo) * 3,
...
void vulkan_material_shader_update_global_state(vulkan_context *context,
    ...
    if (!shader->descriptor_updated[image_index]) { // Configure the descriptors
                                                    // for the given index
...
void vulkan_material_shader_apply_material(vulkan_context *context,
    ...
    vulkan_material_shader_instance_state *object_state =
    ...
    kzero_memory(descriptor_writes,
    ...
    u64 offset = sizeof(vulkan_material_shader_instance_ubo) *
...
b8 vulkan_material_shader_acquire_resources(vulkan_context *context,
                                            vulkan_material_shader *shader,
                                            material *mat) {
    // TODO: free list
    ...
    vulkan_material_shader_instance_state *instance_state =
