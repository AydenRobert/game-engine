...
b8 vulkan_ui_shader_create(vulkan_context *context,
                           vulkan_ui_shader *out_shader) {
    char stage_type_strs[UI_SHADER_STAGE_COUNT][5] = {"vert", "frag"};
    VkShaderStageFlagBits stage_types[UI_SHADER_STAGE_COUNT] = {
    ...
    // Global descriptor pool; used for items like view/projection maticies.
    ...
    kzero_memory(&bindings, sizeof(VkDescriptorSetLayoutBinding) *
...
#define ATTRIBUTE_COUNT 2
    ...
    u64 sizes[ATTRIBUTE_COUNT] = {sizeof(vec2), sizeof(vec2)};
    ...
    // Stages
    // NOTE: should match number of shader->stages
    ...
    if (!vulkan_graphics_pipeline_create(
            context, &context->ui_renderpass, sizeof(vertex_2d),
            ATTRIBUTE_COUNT, attribute_descriptions,
            descriptor_set_layout_count, layouts, UI_SHADER_STAGE_COUNT,
    ...
    if (!vulkan_buffer_create(context, sizeof(vulkan_ui_shader_global_ubo) * 3,
...
void vulkan_ui_shader_destroy(vulkan_context *context,
                              vulkan_ui_shader *shader) {

    vkDestroyDescriptorPool(context->device.logical_device,
                            shader->object_descriptor_pool, context->allocator);
    ...
    vkDestroyDescriptorPool(context->device.logical_device,
                            shader->global_descriptor_pool, context->allocator);
...
void vulkan_ui_shader_update_global_state(vulkan_context *context,
                                          vulkan_ui_shader *shader,
                                          f32 delta_time) {
    u32 image_index = context->image_index;
    ...
    vulkan_buffer_load_data(context, &shader->global_uniform_buffer, offset,
                            range, 0, &shader->global_ubo);
...
void vulkan_ui_shader_set_model(vulkan_context *context,
                                vulkan_ui_shader *shader, mat4 model) {
    if (!context || !shader) {
        return;
    }
...
void vulkan_ui_shader_apply_material(vulkan_context *context,
                                     vulkan_ui_shader *shader,
                                     material *material) {
    if (!context || !shader) {
        return;
    ...
    vulkan_ui_shader_instance_state *object_state =
        &shader->instance_states[material->internal_id];
    VkDescriptorSet object_descriptor_set =
        object_state->descriptor_sets[image_index];

    // TODO: if needs update - might not have to run the following code
    VkWriteDescriptorSet descriptor_writes[VULKAN_UI_SHADER_DESCRIPTOR_COUNT];
    kzero_memory(descriptor_writes, sizeof(VkWriteDescriptorSet) *
                                        VULKAN_UI_SHADER_DESCRIPTOR_COUNT);
    ...
    u64 offset = sizeof(vulkan_ui_shader_instance_ubo) *
                 material->internal_id; // index into array
    ...
    vulkan_buffer_load_data(context, &shader->object_uniform_buffer, offset,
                            range, 0, &instance_ubo);

    // Only do this if descriptor has not been updated
    ...
    for (u32 sampler_index = 0; sampler_index < sampler_count;
        ...
        // If texture hasn't been loaded, use default
        if (!t) {
        ...
        // Check to see if descriptor needs updating
        if (t && (*descriptor_id != t->id ||
            ...
            // Sync frame generation if not using default texture
    ...
    vkCmdBindDescriptorSets(command_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            shader->pipeline.pipeline_layout, 1, 1,
                            &object_descriptor_set, 0, 0);
}

b8 vulkan_ui_shader_acquire_resources(vulkan_context *context,
                                      vulkan_ui_shader *shader, material *mat) {
    // TODO: free list
    mat->internal_id = shader->object_uniform_buffer_index;
    shader->object_uniform_buffer_index++;

    vulkan_ui_shader_instance_state *instance_state =
        &shader->instance_states[mat->internal_id];
    for (u32 i = 0; i < VULKAN_UI_SHADER_DESCRIPTOR_COUNT; i++) {
        for (u32 j = 0; j < 3; j++) {
...
void vulkan_ui_shader_release_resources(vulkan_context *context,
                                        vulkan_ui_shader *shader,
                                        material *mat) {
    ...
    VkResult result = vkFreeDescriptorSets(
        context->device.logical_device, shader->object_descriptor_pool,
        descriptor_set_count, instance_state->descriptor_sets);
    if (result != VK_SUCCESS) {
