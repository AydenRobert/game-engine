...
void cleanup_freelist(vulkan_buffer *buffer) {
    ...
    kfree(buffer->freelist_block, buffer->freelist_memory_requirement,
          MEMORY_TAG_RENDERER);
...
b8 vulkan_buffer_create(vulkan_context *context, u64 size,
    ...
    out_buffer->freelist_block =
        kallocate(out_buffer->freelist_memory_requirement, MEMORY_TAG_RENDERER);
    ...
    // Allocate the memory
    VkResult result =
        vkAllocateMemory(context->device.logical_device, &allocate_info,
...
b8 vulkan_buffer_resize(vulkan_context *context, u64 new_size,
    ...
    if (new_size < buffer->total_size) {
        KERROR("vulkan_buffer_resize requires the new size be larger than the "
               "old size.");
        return false;
    }
    ...
    freelist_resize(&buffer->buffer_freelist, &new_memory_requirement, 0,
                    new_size, 0);
    ...
    // Copy over the data
    vulkan_buffer_copy_to(context, pool, 0, queue, buffer->handle, 0,
                          new_buffer, 0, buffer->total_size);

    // Make sure anything using these is potentially finished
...
void vulkan_buffer_bind(vulkan_context *context, vulkan_buffer *buffer,
                        u64 offset) {
    VK_CHECK(vkBindBufferMemory(context->device.logical_device, buffer->handle,
                                buffer->memory, offset));
}

void *vulkan_buffer_lock_memory(vulkan_context *context, vulkan_buffer *buffer,
                                u64 offset, u64 size, u32 flags) {
    void *data;
    VK_CHECK(vkMapMemory(context->device.logical_device, buffer->memory, offset,
                         size, flags, &data));
    return data;
}
...
b8 vulkan_buffer_allocate(vulkan_buffer *buffer, u64 size, u64 *out_offset) {
    if (!buffer || !size || !out_offset) {
        KERROR("vulkan_buffer_allocate - required valid buffer, size and "
               "out_offset.");
        return false;
    }
...
b8 vulkan_buffer_free(vulkan_buffer *buffer, u64 size, u64 offset) {
    if (!buffer || !size) {
        KERROR("vulkan_buffer_allocate - required valid buffer and size.");
        return false;
    }
...
void vulkan_buffer_copy_to(vulkan_context *context, VkCommandPool pool,
    ...
    vulkan_command_buffer_end_single_use(context, pool, &temp_command_buffer,
                                         queue);
}
