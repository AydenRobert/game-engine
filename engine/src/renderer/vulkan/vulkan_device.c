...
b8 vulkan_device_create(vulkan_context *context) {
    ...
    device_features.samplerAnisotropy = true; // Request anisotropy
    ...
    KINFO("Graphics command pool has been created.");
...
void vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (!out_support_info->formats) {
            out_support_info->formats = darray_reserve(
                VkSurfaceFormatKHR, out_support_info->format_count);
        }
    }
    VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
    ...
    if (out_support_info->present_mode_count != 0) {
        if (!out_support_info->present_modes) {
            out_support_info->present_modes = darray_reserve(
                VkPresentModeKHR, out_support_info->present_mode_count);
        }
    }
    VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
...
b8 select_physical_device(vulkan_context *context) {
    ...
    if (physical_device_count == 0) {
        KFATAL("No Vulkan compatible physical devices were found.");
        return false;
    }
    ...
    for (u32 i = 0; i < physical_device_count; i++) {
        ...
        if (result) {
            ...
            case VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU:
                KINFO("Device type integrated.");
                break;
            case VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU:
                KINFO("Device type discrete.");
                break;
            case VK_PHYSICAL_DEVICE_TYPE_VIRTUAL_GPU:
                KINFO("Device type virtual.");
                break;
...
b8 physical_device_meets_requirements(
    ...
    out_queue_info->graphics_family_index =
        -1; // large value that doesn't make sense
    ...
    if (requirements->discrete_gpu) {
        if (properties->deviceType != VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
            KINFO("Device is not a Discrete GPU, one is required. Skipping.");
            return false;
        }
    }
    ...
    for (u32 i = 0; i < queue_family_count; i++) {
        ...
        if (queue_families[i].queueFlags & VK_QUEUE_TRANSFER_BIT) {
            // Take the index if it is the lowest
    ...
    if ((!requirements->graphics ||
        ...
        if (requirements->device_extension_names) {
            ...
            if (available_extension_count != 0) {
                available_extensions = darray_reserve(
                    VkExtensionProperties, available_extension_count);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
