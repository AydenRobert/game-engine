...
void vulkan_swapchain_destroy(vulkan_context *context,
    ...
    for (u32 i = 0; i < swapchain->image_count; i++) {
        kfree(swapchain->render_textures[i]->internal_data,
              sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
    }
}
...
void vulkan_swapchain_present(vulkan_context *context,
    ...
    VkSemaphore render_finished =
        swapchain->render_finished_per_image[present_image_index];
...
void create(vulkan_context *context, u32 width, u32 height,
    ...
    // Clamp to the value allow by the GPU
    VkExtent2D min =
    ...
    swapchain_extent.width =
        KCLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height =
        KCLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (context->device.graphics_queue_index !=
        ...
        u32 queue_family_indices[] = {(u32)context->device.graphics_queue_index,
    ...
    if (!swapchain->render_textures) {
        // TODO: fix
        swapchain->render_textures = (texture **)kallocate(
            sizeof(texture *) * swapchain->image_count, MEMORY_TAG_RENDERER);

        for (u32 i = 0; i < swapchain->image_count; i++) {
            void *internal_data =
                kallocate(sizeof(vulkan_image), MEMORY_TAG_TEXTURE);
            ...
            swapchain->render_textures[i] = texture_system_wrap_internal(
                tex_name, swapchain_extent.width, swapchain_extent.height, 4,
                false, true, false, internal_data);
        ...
        for (u32 i = 0; i < swapchain->image_count; i++) {
            texture_system_resize(swapchain->render_textures[i],
                                  swapchain_extent.width,
                                  swapchain_extent.height, false);
        }
    }
    ...
    for (u32 i = 0; i < swapchain->image_count; i++) {
        vulkan_image *image =
            (vulkan_image *)swapchain->render_textures[i]->internal_data;
    ...
    if (!swapchain->render_finished_per_image) {
        swapchain->render_finished_per_image = (VkSemaphore *)kallocate(
            sizeof(VkSemaphore) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    VkSemaphoreCreateInfo sci = {.sType =
                                     VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO};
    for (u32 i = 0; i < swapchain->image_count; ++i) {
        VK_CHECK(vkCreateSemaphore(context->device.logical_device, &sci,
                                   context->allocator,
                                   &swapchain->render_finished_per_image[i]));
    }
    ...
    vulkan_image_create(context, VK_IMAGE_TYPE_2D, swapchain_extent.width,
                        ...
                        VK_IMAGE_ASPECT_DEPTH_BIT, image);

    context->swapchain.depth_texture = texture_system_wrap_internal(
        "__kohi_default_depth_texture__", swapchain_extent.width,
        swapchain_extent.height, context->device.depth_channel_count, false,
        true, false, image);
...
void destroy(vulkan_context *context, vulkan_swapchain *swapchain) {
    // Ensure GPU is idle before destroying swapchain-dependent resources
    vkDeviceWaitIdle(context->device.logical_device);

    vulkan_image_destroy(
        context, (vulkan_image *)swapchain->depth_texture->internal_data);
    kfree(swapchain->depth_texture->internal_data, sizeof(vulkan_image),
          MEMORY_TAG_TEXTURE);
    context->swapchain.depth_texture = 0;

    // Only destroy the view, not the images. Those are owned by the swapchain.
    for (u32 i = 0; i < swapchain->image_count; i++) {
        vulkan_image *image =
            (vulkan_image *)swapchain->render_textures[i]->internal_data;
    ...
    if (swapchain->render_finished_per_image) {
        for (u32 i = 0; i < swapchain->image_count; ++i) {
            if (swapchain->render_finished_per_image[i]) {
                vkDestroySemaphore(context->device.logical_device,
                                   swapchain->render_finished_per_image[i],
                                   context->allocator);
            }
        }
        kfree(swapchain->render_finished_per_image,
              sizeof(VkSemaphore) * swapchain->image_count,
              MEMORY_TAG_RENDERER);
        swapchain->render_finished_per_image = 0;
    }
