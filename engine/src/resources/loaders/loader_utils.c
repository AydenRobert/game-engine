...
b8 resource_unload(struct resource_loader *self, resource *resource,
                   memory_tag tag) {
    if (!self) {
        KWARN("resource_unload called with nullptr for self.");
        return false;
    }
    ...
    if (path_length) {
        kfree(resource->full_path, sizeof(char) * (path_length + 1),
              MEMORY_TAG_STRING);
    }
