...
b8 material_loader_load(struct resource_loader *self, const char *name,
    ...
    while (filesystem_read_line(&file, 511, &p, &line_length)) {
        ...
        } else if (strings_equali(trimmed_variable_name, "specular_map_name")) {
            string_ncopy(resource_data->specular_map_name, trimmed_value,
                         TEXTURE_NAME_MAX_LENGTH);
        } else if (strings_equali(trimmed_variable_name, "normal_map_name")) {
            string_ncopy(resource_data->normal_map_name, trimmed_value,
                         TEXTURE_NAME_MAX_LENGTH);
        ...
        } else if (strings_equali(trimmed_variable_name, "shininess")) {
            if (!string_to_f32(trimmed_value, &resource_data->shininess)) {
                KWARN("Error parsing shininess in file '%s'. Using default of "
                      "32.0 instead.",
                      full_file_path);
                resource_data->shininess = 32.0f;
            }
