...
b8 import_obj_file(file_handle *obj_file, const char *out_ksm_filename,
                   geometry_config **out_geometries_darray);
void process_subobject(vec3 *positions, vec3 *normals, vec2 *tex_coords,
                       mesh_face_data *faces, geometry_config *out_data);
b8 import_obj_material_library_file(const char *mtl_file_path);

b8 load_ksm_file(file_handle *ksm_file,
                 geometry_config **out_geometries_darray);
b8 write_ksm_file(const char *path, const char *name, u32 geometry_count,
                  geometry_config *geometries);
b8 write_kmt_file(const char *mtl_file_path, material_config *config);

b8 mesh_loader_load(struct resource_loader *self, const char *name,
...
#define SUPPORTED_FILETYPE_COUNT 2
    ...
    if (type == MESH_FILE_TYPE_NOT_FOUND) {
        KERROR("Mesh loader could not find file with name '%s'.", name);
        return false;
    }
    ...
    case MESH_FILE_TYPE_NOT_FOUND:
        KERROR("Unable to find mesh of supported type with name '%s'.", name);
        result = false;
        break;
    }
...
b8 load_ksm_file(file_handle *ksm_file,
    ...
    for (u32 i = 0; i < geometry_count; i++) {
        ...
        g.vertices =
            kallocate(g.vertex_size * g.vertex_count, MEMORY_TAG_ARRAY);
        ...
        // Name
        u32 g_name_len = 0;
        filesystem_read(ksm_file, sizeof(u32), &g_name_len, &bytes_read);
        filesystem_read(ksm_file, sizeof(char) * g_name_len, g.name,
                         &bytes_read);

        // Material Name
        u32 g_material_name_len = 0;
        filesystem_read(ksm_file, sizeof(u32), &g_material_name_len,
                         &bytes_read);
        filesystem_read(ksm_file, sizeof(char) * g_material_name_len,
                         g.material_name, &bytes_read);

        // Centre
        filesystem_read(ksm_file, sizeof(vec3), &g.centre, &bytes_read);

        // Extents (Min Extent|Max Extent)
...
b8 write_ksm_file(const char *path, const char *name, u32 geometry_count,
    ...
    for (u32 i = 0; i < geometry_count; i++) {
        ...
        filesystem_write(&f, g->index_size * g->index_count, g->indices,
                         &written);

        // Name
        u32 g_name_len = string_length(g->name);
        filesystem_write(&f, sizeof(u32), &g_name_len, &written);
        filesystem_write(&f, sizeof(char) * g_name_len, g->name, &written);

        // Material Name
        u32 g_material_name_len = string_length(g->material_name);
        filesystem_write(&f, sizeof(u32), &g_material_name_len, &written);
        filesystem_write(&f, sizeof(char) * g_material_name_len,
                         g->material_name, &written);

        // Centre
        filesystem_write(&f, sizeof(vec3), &g->centre, &written);
...
KINLINE void process_obj_group(vec3 *positions, vec3 *normals, vec2 *tex_coords,
                               mesh_group_data *groups, char *name,
                               char material_names[32][64], u64 i,
                               geometry_config **out_geometries_darray) {
    geometry_config new_data = {};
    string_ncopy(new_data.name, name, GEOMETRY_NAME_MAX_LENGTH - 1);

    if (i > 0) {
        // Avoid duplicate names
        string_append_int(new_data.name, new_data.name, i);
    }
    string_ncopy(new_data.material_name, material_names[i],
                 MATERIAL_NAME_MAX_LENGTH - 1);
...
b8 import_obj_file(file_handle *obj_file, const char *out_ksm_filename,
                   geometry_config **out_geometries_darray) {
    const u32 initial_array_size = 16384;
    vec3 *positions = darray_reserve(vec3, initial_array_size);
    vec3 *normals = darray_reserve(vec3, initial_array_size);
    vec2 *tex_coords = darray_reserve(vec2, initial_array_size);
    // Groups of faces
    ...
    while (true) {
        ...
        case '#':
            ...
            continue;
        case 'v': {
            char second_char = lin_buf[1];
            switch (second_char) {
            case ' ': {
                // Vertex position
                vec3 pos;
                char t[2];
                sscanf(lin_buf, "%s %f %f %f", t, &pos.x, &pos.y, &pos.z);
            ...
            case 'n': {
                ...
                vec3 norm;
                char t[2];
                sscanf(lin_buf, "%s %f %f %f", t, &norm.x, &norm.y, &norm.z);
            ...
            case 't': {
                ...
                vec2 tex_coord;
                char t[2];
                sscanf(lin_buf, "%s %f %f", t, &tex_coord.x, &tex_coord.y);
        ...
        case 'm': {
            ...
            char substr[7];
            sscanf(lin_buf, "%s %s", substr, material_file_name);

            // if found, save off material file name
            if (strings_nequal(substr, "mtllib", 6)) {
                // TODO: verification
            }
        } break;
        case 'u': {
            // Any time there is a usemtl, assume new group
            mesh_group_data new_group;
            new_group.faces =
                darray_reserve(mesh_face_data, initial_array_size);
            darray_push(groups, new_group);

            char t[8];
            sscanf(lin_buf, "%s %s", t,
                   material_names[current_mat_name_count++]);
        } break;
        case 'g': {
            // Process each group as a sub-object
            u64 group_count = darray_length(groups);
            for (u64 i = 0; i < group_count; i++) {
                process_obj_group(positions, normals, tex_coords, groups, name,
                                  material_names, i, out_geometries_darray);
    ...
    for (u64 i = 0; i < count; i++) {
        ...
        KDEBUG("Geometry de-duplication started on '%s'.", g->name);

        u32 new_vert_count = 0;
        vertex_3d *unique_verts = 0;
        geometry_deduplicate_vertices(g->vertex_count, g->vertices,
                                      g->index_count, g->indices,
                                      &new_vert_count, &unique_verts);

        // Destroy the old geometry
        darray_destroy(g->vertices);

        g->vertex_count = new_vert_count;
        g->vertices = unique_verts;

        // Take a copy of the indices (non-darray)
        u32 *indices =
            kallocate(sizeof(u32) * g->index_count, MEMORY_TAG_ARRAY);
...
void process_subobject(vec3 *positions, vec3 *normals, vec2 *tex_coords,
    ...
    if (normal_count == 0) {
        KWARN("No normals present on this model.");
        skip_normals = true;
    }

    if (tex_coord_count == 0) {
        KWARN("No tex_coords present on this model.");
    ...
    for (u8 i = 0; i < 3; i++) {
        out_data->centre.elements[i] = (out_data->min_extents.elements[i] +
    ...
    while (true) {
        ...
        case 'm': {
            ...
            if (strings_nequali(substr, "map_Kd", 6)) {
                ...
                string_filename_no_extension_from_path(
                    current_config.diffuse_map_name, texture_file_name);
            } else if (strings_nequali(substr, "map_Ks", 6)) {
                // specular
                string_filename_no_extension_from_path(
                    current_config.specular_map_name, texture_file_name);
            } else if (strings_nequali(substr, "map_bump", 8)) {
                // normal
                string_filename_no_extension_from_path(
                    current_config.normal_map_name, texture_file_name);
            }
        } break; // m
        case 'b': {
        ...
        case 'n': {
            ...
            if (strings_nequali(substr, "newmtl", 6)) {
                // material name
                // NOTE: Treating all material imported here the same
                current_config.shader_name = BUILTIN_SHADER_NAME_MATERIAL;
...
b8 write_kmt_file(const char *mtl_file_path, material_config *config) {
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_WRITE, false, &f)) {
        KERROR("Error opening material file for writing: '%s'.",
               full_file_path);
        return false;
    }
    ...
    string_format(line_buffer, "diffuse_colour=%.6f %.6f %.6f %.6f",
                  config->diffuse_colour.r, config->diffuse_colour.g,
                  config->diffuse_colour.b, config->diffuse_colour.a);
    filesystem_write_line(&f, line_buffer);
    ...
    if (config->diffuse_map_name[0]) {
        string_format(line_buffer, "diffuse_map_name=%s",
                      config->diffuse_map_name);
        filesystem_write_line(&f, line_buffer);
    }
    if (config->specular_map_name[0]) {
        string_format(line_buffer, "specular_map_name=%s",
                      config->specular_map_name);
        filesystem_write_line(&f, line_buffer);
    }
    if (config->normal_map_name[0]) {
        string_format(line_buffer, "normal_map_name=%s",
