...
b8 text_loader_load(struct resource_loader *self, const char *name,
    ...
    if (!filesystem_open(full_file_path, FILE_MODE_READ, false, &file)) {
        KERROR("text_loader_load - unable to open text file for reading: "
               "'%s'.",
               full_file_path);
    }
    ...
    // TODO: should use an allocator
    char *resource_data = kallocate(sizeof(u8) * file_size, MEMORY_TAG_ARRAY);
    u64 read_size = 0;
