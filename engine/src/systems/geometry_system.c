...
geometry_config
geometry_system_generate_cube_config(f32 width, f32 depth, f32 height,
    ...
    config.vertices =
        kallocate(sizeof(vertex_3d) * config.vertex_count, MEMORY_TAG_SCENE);
    config.index_size = sizeof(u32);
    config.index_count = 6 * 6;
    config.indices =
        kallocate(sizeof(u32) * config.index_count, MEMORY_TAG_SCENE);
    ...
    kcopy_memory(config.vertices, verts,
